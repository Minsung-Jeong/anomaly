...
def RNNmodel(x_rnn, y_rnn, trn_prop, index):
    ...
    val_size = round((len(x_rnn)-train_size)/2)

    x_train = tf.convert_to_tensor(x_rnn[:train_size])
    y_train = tf.convert_to_tensor(y_rnn[:train_size])
    # x_val = tf.convert_to_tensor(x_rnn[train_size: train_size + val_size])
    # y_val = tf.convert_to_tensor(y_rnn[train_size: train_size + val_size])
    # x_test = tf.convert_to_tensor(x_rnn[train_size + val_size:])
    # y_test = tf.convert_to_tensor(y_rnn[train_size + val_size:])
    x_test = tf.convert_to_tensor(x_rnn[train_size:])
    y_test = tf.convert_to_tensor(y_rnn[train_size:])
    tst_index = index[train_size:]
    ...
    return pred, mean_absolute_error(y_test, pred),  y_test, tst_index
...
y_rnn = y_cut[:-seq_len] #seq_len 단위로 예측하다 보니 뒤쪽은 삭제해야함
