...
class LSTM_VAE(tf.keras.Model):
    def __init__(self, time_step, x_dim, lstm_h_dim, z_dim, name='encoder', **kwargs):
        ...
        self.encoder = tf.keras.Sequential(
            [
                tf.keras.InputLayer(input_shape=(time_step, x_dim)),
                tf.keras.layers.LSTM(lstm_h_dim, activation='softplus', name='encoder_lstm'),
                tf.keras.Dense(z_dim + z_dim)
            ]
        )
    ...
    def call(self, x):
        ...
        mu_x, sigma_x = tf.split(self.self.decoder(z), num_or_size_splits=2, axis=1)
...
def train_step(model, data, optimizer):
    ...
    with tf.GradientTape() as tape:
        mu_x, sigma_x, log_px = model(x)
        loss = model.reconstruct_loss(x, mu_x, sigma_x)
        loss += sum(model.losses)
        mean_log_px = model.mean_log_likelihood(log_px)
